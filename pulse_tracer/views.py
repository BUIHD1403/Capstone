...
class IndexView(LoginRequiredMixin, generic.ListView):
    ...
    def get(self, request, **kwargs):
        ...
        if user.is_patient:
            ...
            #Assume patient= health_care_provider__user__id =5
        ...
        self.template_name = 'pulse_tracer/index.html'
        ...
        lastItem=HeartRate.objects.filter(analyzed_time__date__range=(week_prior, today) )
        print(lastItem)
        if not lastItem: 
                hr_labels, hr_data, rr_data = query_scripts.get_weekly_summary(user)
                context = {
        ...
        lastItemRR=RespiratoryRate.objects.filter(analyzed_time__date__range=(week_prior, today) )
        ...
        '''  # Remove for Patient view
        #patients = Patient.objects.filter(health_care_provider__user__id=request.user.id)
        #patients = Patient.objects.all()
        patients = Patient.objects.filter(health_care_provider__user__id=request.user.id)
        if not patients:
            print("NOOOOOOOONEEEE")
            patients = Patient.objects.all()
        print("--------------------Query Set------------------")
        print(patients.query)
        #patients = patients.filter(id=1)
        df = pd.DataFrame(list(patients.values()))       
        dfUser=df['user_id']
       
        dfHR = pd.DataFrame(list(HeartRate.objects.all().values()))    
        
        df3 = pd.merge(dfUser, dfHR, left_on='user_id', right_on='patient_id', how='inner')
        dfResult=df3.sort_values('analyzed_time').groupby('patient_id').tail(1)

        #dfHR = pd.merge(dfUser, dfHR, dfHR='user_id', how='inner')
        print("--------------------Dataframe------------------")
        print(df3)
        print("--------------------Dataframe Result------------------")
        print(dfResult)
        arr= dfResult['heart_rate'].values.tolist()
        print(arr)
...
class DataSummaryView(LoginRequiredMixin, generic.TemplateView):
    ...
    def get(self, request, **kwargs):
        current_user_id = request.user.id
        #NOT SURE IF NEED TO REMOVE
        #patient = get_object_or_404(Patient, user__id=current_user_id)

        #TO DO: need to process if user select no data day range
        print(current_user_id)
        min_day= request.GET.get('datefrom') or '2019-11-20'
        max_day = request.GET.get('dateto') or '2019-11-21'
        ...
        print("SUM HRRRRRRRRRRRR")
        dfHR = pd.DataFrame(list(sumHR.values()))
        print(dfHR)
        #print(sumHR.query)
        #print(sumHR)
